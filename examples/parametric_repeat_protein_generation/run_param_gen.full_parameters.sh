...
#
# CAUTION: instead of scanning all the parameters below on a single cpu, 
#          we suggest splitting them and running each parameter combination on a cpu on the cluster
#
...
for h1_len in ${H1_LENGTH}; do # length of helix1 (h1)
  for r_z1_d in 30; do # stride of 'self-rotation degree' of h1 about the Z axis
    for r_x1 in -20 -10 0 10 20; do  # rotation degree of h1 about the radius line
      for r_y1 in -20 -10 0 10 20; do  # rotation degree of h1 about the tangent line
        for t_x1 in 25 30 35; do # (radius) translation of h1 along X axis
          for d12 in 8.5 9 9.5 10 10.5; do  # distance between inner row helix (h1), this term with t_x1 overwrite r_zr
            for handedness in -1 1; do  # handedness of dhr
              for h2_len in ${H2_LENGTH}; do # length of helix2 (h2)
                for r_z2_d in 30; do # stride of 'self-rotation degree' of h2 about the Z axis
                            ...
                            for r_yr in 0; do  # (Kobe's repeat twist) rotation degree of r2 from r1 about intra-dhr helical axis
                              for t_xr in 0; do  # translation of repeat2 (r2) from r1 along X axis
                                for t_yr in 0; do  # translation of repeat2 (r2) from r1 along Y axis
                                  for t_zr in -4 -2 0 2 4; do  # (rise) translation of repeat2 (r2) from r1 along Z axis
